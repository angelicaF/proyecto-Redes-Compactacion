...
int main(int argc, char *argv[]) {
   ...
   while (x<30){
     ...
     //ci.getTime(); // Start the time measurement
      ...
      stopT = clock();
      double tiempoTranscurrido=((stopT-startT)/CLOCKS_PER_SEC);
   ...
   */
      fclose(file);  
      buff_datos[x]=tiempoTranscurrido;
      fwrite(buff_datos,sizeof(char),sizeof(buff_datos),f);
      x++;
   }
   close (f);
   return 0;
